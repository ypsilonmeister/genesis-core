...
async fn main() -> Result<()> {
    ...
    if addr_or_path.starts_with("tcp://") {
        ...
        #[cfg(not(unix))]
        {
            anyhow::bail!("Unix Domain Sockets are not supported on this platform: {}", addr_or_path);
        }
    }
}
                ...
                if let Ok(payload) = serde_json::from_str::<Value>(&line) {
                    ...
                    if let Some(input) = payload.get("input").and_then(|v| v.as_str()) {
                        if input.contains("plus") || input.contains("足す") || input.contains("times") || input.contains("かける")
                            || input.contains("##") || input.contains("log") || input.contains('\'') 
                            || input.matches("**").count() > 1 || input.contains(") ** (") || input.contains(")**(") {
                            error = Some(json!({
                                "code": "UNKNOWN_PATTERN",
                                "message": format!("Unsupported pattern detected: {}", input),
                                "input_position": 0
                            }));
                        } else {
